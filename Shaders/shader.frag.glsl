...
in mat4 sg_mat4;
vec3 sw_transform4(vec3 p){
return (sg_mat4 * vec4(p, 1.0)).xyz;
}
vec4 sg_node4(vec3 p);vec4 sg_node4(vec3 p) {p = sw_transform4(p);return vec4(sg_data[12][0],sg_data[12][1],sg_data[12][2],sd_box(p,vec3(sg_data[11][1],sg_data[11][2],sg_data[11][3])));}
